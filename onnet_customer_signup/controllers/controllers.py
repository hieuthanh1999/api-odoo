...
class Signup(http.Controller):
    ...
    def customer_pricing_plans(self, **kw):
        ...
        pricelist = request.env['product.pricelist'].browse(
            int(kw.get('pricelist_id')))
        plan_active = request.env['product.template'].sudo().search([('id', '=', kw.get('plan'))])
        addons = request.env['product.template'].sudo().search([('is_trial', '=', False), ('is_add_ons', '=', True)], order='sequence ASC')

        if kw.get('trial') and kw.get('trial') == '1':
            trial = kw.get('trial')
            addons = request.env['product.template'].sudo().search([('is_trial', '=', True), ('is_add_ons', '=', True)])
        max_number_user = request.env['ir.config_parameter'].sudo().get_param('onnet_custom_subscription.max_number_user')
        values = {
            'industry_id': int(kw.get('industry')),
            'pricelist': pricelist,
            'product': plan_active,
            'trial': int(trial),
            'add_ons': addons,
        ...
        return http.request.render('onnet_customer_signup.template_plans', values)
    ...
    def get_addon_ajax(self, trial, product_order, quantity, product_id):
        if trial == '1':
            addons = request.env['product.template'].sudo().search([('is_trial', '=', True), ('is_add_ons', '=', True)])
        else:
            addons = request.env['product.template'].sudo().search(
                [('is_trial', '=', False), ('is_add_ons', '=', True)])
        line_ids = []
        if product_order:
            line_ids = product_order.split(',')
            line_ids = [int(numeric_string) for numeric_string in line_ids]
        plan = request.env['product.template'].sudo().search([('id', '=', int(product_id))])
        values = {
            'html': request.env.ref('onnet_customer_signup.view_template_user_addon')._render({
                'add_ons': addons,
                'trial': int(trial),
    ...
    def get_addon_ajax_step2(self, trial,  order_id=None):
        ...
        domain = ""
        if order_id:
            domain = request.env['sale.subscription'].sudo().search([('order_id', '=', int(order_id))]).website
            domain = domain.replace(".hiiboss.com", "")
    ...
    def get_addon_ajax_step3(self, trial, reg_name, reg_email, reg_phone, reg_company_name, reg_country,
                             reg_language_id, reg_domain, product_order, product_id, number_user, reg_industry,
                             pricelist_id, code_country, reg_state, zipcode, reg_street, order_id=None):
        pricelist = request.env['product.pricelist'].browse(int(pricelist_id))

        phone_new = reg_phone.lstrip("+"+code_country)
        reg_phone = "+"+code_country+phone_new
        if request.env.user.partner_id.id:
            partner_id = int(request.env.user.partner_id.id)
            mode = ('edit', 'billing')
        if partner_id == 4:
        ...
        # Stage
        trial_id = int(
            request.env['ir.config_parameter'].sudo().get_param('onnet_custom_subscription.sale_subscription_trial'))
        draft_id = int(
            request.env['ir.config_parameter'].sudo().get_param('onnet_custom_subscription.sale_subscription_draft'))
        if int(trial) == 1:
            id_stage = trial_id
        else:
            id_stage = draft_id
        ...
        if int(trial) == 1:
            ...
            if order_id:
                ...
                subscription_order = request.env['sale.subscription'].sudo().search([('order_id', '=', int(order_id))], limit=1)
            ...
            values = {
                'trial': int(trial),
                'order_id': order.id,
                'html': request.env.ref('onnet_customer_signup.payment_ajax')._render(render_values)
            }
    ...
    def show_addon_template_ajax(self, data):
        addons_recurring = request.env['product.template'].sudo().search(
            [('is_recurring', '=', True), ('is_add_ons', '=', True), ('id', 'in', data['list_add_on'])])
        addons = request.env['product.template'].sudo().search(
            [('is_recurring', '=', False), ('is_add_ons', '=', True), ('id', 'in', data['list_add_on'])])
        pricelist = request.env['product.pricelist'].browse(int(data['pricelist_id']))
        values = {}
        if addons or addons_recurring:
            values.update({
                'html': request.env.ref('onnet_customer_signup.view_addons_summary')._render({
    ...
    # get - peicelist - plan
    @http.route('/plans/get-peicelist-plan', csrf=False, type='json', auth='public', website=True)
    def detail_sale_order(self, data):
        ...
        pricelist = request.env['product.pricelist'].browse(int(data['pricelist_id']))
        max_number_user = int(request.env['ir.config_parameter'].sudo().get_param('onnet_custom_subscription.max_number_user'))
    ...
    @http.route('/plans/step_payment', csrf=False, type='json', auth='public', website=True)
    def step_payment(self, trial):
        order = request.env['sale.order'].sudo().search([('id', '=', 1072)])
        render_values = {
            'trial': int(trial)
        }
        if order:
