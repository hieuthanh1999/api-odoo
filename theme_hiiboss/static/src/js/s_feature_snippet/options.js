odoo.define('theme_hiiboss.s_feature_slider_options', function (require) {
    "use strict";
    ...
    options.registry.FeatureSlider= options.Class.extend({
        xmlDependencies: [ '/theme_hiiboss/static/src/xml/feature_dialog.xml'],
        ...
        _service_configure: function(){
            ...
            var $modal = $(qweb.render('theme_hiiboss.feature_dialog_template'));
            $modal.modal();
            cr._rpc({
                route: '/get_feature_slider_collection',
                params: {
                    'collection_id':cr.$target.attr("data-collection-id"),
                }
            }).then(function (data) {
                var service_col_ele =  $modal.find('select[name="feature_collection"]');
                var collection_name = "";
                if(data.length > 0){
                ...
                $modal.on('click', '.btn_apply', function(e){
                    e.preventDefault();
                    var selected_collection = $modal.find('select[name="feature_collection"]').val();
